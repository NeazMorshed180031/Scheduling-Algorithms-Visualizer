...
public class Schedule extends Application {
    ...
    public void start(Stage primaryStage) {
        primaryStage.setTitle("Scheduling Algorithms Visualizer by Neaz Morshed");
    ...
    private void runFCFSScheduling() {
        Task<Void> task = new Task<>() {
            ...
            protected Void call() {
                ...
                new FCFS().schedule(processes, db, Schedule.this);
                return null;
            }
        };
    ...
    private void runSJFScheduling() {
        Task<Void> task = new Task<>() {
            ...
            protected Void call() {
                ...
                List<Process> processes = getSampleProcesses();
                new SJF().schedule(processes, db, Schedule.this);
                return null;
            }
        };
        new Thread(task).start();
    ...
    private void runRRScheduling() {
        Task<Void> task = new Task<>() {
            ...
            protected Void call() {
                ...
                List<Process> processes = getSampleProcesses();
                new RoundRobin().schedule(processes, 2, db, Schedule.this);
                return null;
            }
        };
        new Thread(task).start();
...
class FCFS {
    public void schedule(List<Process> processes, Database db, Schedule main) {
        int currentTime = 0;
...
class RoundRobin {
    public void schedule(List<Process> processes, int quantum, Database db, Schedule main) {
        int currentTime = 0;
        int totalWaitingTime = 0;
        int totalTurnaroundTime = 0;
